...
EXPERIMENT_NAME = 'Genre Detection with fastText'
...
def download_feedback_data(feedback_url):
    ...
    Get data from the feedback module per model ID
    and return it in a format that can be used directly in training data.
    TODO: check the minimum length of a feedback text,
    and whether it exists in the training or test data.
    """
    feedback = []
    try:
...
def train(csv_file, feedback_url, model_id=None):
    exp_id = get_or_create_experiment_id()
    with mlflow.start_run(experiment_id=exp_id):
        df_1 = pd.read_csv(csv_file)
        # add in the feedback training data

        # todo: find latest model id from the model management system
        ...
        # add feedback
        feedback_cleaned_df = pd.concat([feedback_df, cleaned_df], ignore_index=True)
        print("...add feedback")
        print("Total length of samples: " + str(len(feedback_cleaned_df)))

        # Upsample anything that has less than 100 samples
        upsampled_dfs = []
        cutoff_counts = feedback_cleaned_df['genre'].value_counts().to_dict()
        print(cutoff_counts)
        for label, count in cutoff_counts.items():
            if count < 100:
                print("resampling... " + label)
                df_ = resample(feedback_cleaned_df[feedback_cleaned_df['genre'] == label],
                                replace=True,
                                n_samples=100,
                                random_state=123
                              )
        ...
        train_, test_ = train_test_split(resampled_df, test_size=test_percentage)
        mlflow.log_metric("training data size", len(train_))
        print("length of training data: ", len(train_))
        mlflow.log_metric("testing data size", len(test_))
        ...
        printout_predictions(model, resampled_df)
